...
export const Dashboard: React.FC<DashboardProps> = ({ isLoggedIn, setIsLoggedIn }) => {
  ...
  const formatDateTime = (dateTime: string) => {
    const regex = /^(\d{4})-(\d{2})-(\d{2})/; // Captura o ano, mês e dia
    const match = dateTime.match(regex);
 
    if (match) {
      const [, year, month, day] = match; // Desestrutura os grupos capturados
      return `${day}/${month}/${year}`; // Retorna no formato DD/MM/YYYY
    }
 
    return dateTime; // Retorna o original caso não seja compatível
  };
...
<button className="px-4 py-2 bg-Lala rounded hover:bg-Lalah" onClick={() => navigate("/historypage")}>Histórico</button>
<button className="px-4 py-2 bg-Lala rounded hover:bg-Lalah" onClick={() => navigate("/shoppinglistpage")}>Lista</button>
<button className="px-4 py-2 bg-Lala rounded hover:bg-Lalah" onClick={() => navigate("/reports")}
>
  Relatório
</button>
<button className="px-4 py-2 bg-Lala rounded hover:bg-Lalah" onClick={() => navigate("/settings")}>Configurações</button>
